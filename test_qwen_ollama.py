"""
测试 Ollama Qwen2:7b-instruct 模型在亲属关系任务上的表现
训练前基准测试
"""
...
def test_ollama_model(model_name="qwen2:7b-instruct"):
    print("=" * 70)
    ...
    for i, (query, true_answer) in enumerate(zip(test_queries[:50], test_answers[:50])):
        ...
        print(f"[{status}] {i+1}/50: {query[:35]}...")
        print(f"   A: {model_answer}")
        if not is_correct:
            print(f"   正确: {true_answer}")
        
        if (i + 1) % 10 == 0:
            print(f"\n--- 进度: {i+1}/50, 正确: {correct}, 错误: {wrong} ---\n")
...
if __name__ == "__main__":
    results = test_ollama_model("qwen2:7b-instruct")
    ...
    output_path = "./outputs/qwen2_7b_test_results.json"
